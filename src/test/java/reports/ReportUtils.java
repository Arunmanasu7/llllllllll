...
public class ReportUtils extends DriverScript{
	 ...
	 * Author		: Sg User1
	 * Parameters	: FileName, ScenarioName
	...
	public ExtentReports startReport(String fileName, String scenarioName, String strBuildNum)
	{
		...
		File objBuildNum = null;
		String strBuildNumber = null;
		try {
			strPath = System.getProperty("user.dir")+"\\Results";
			
			strBuildNumber = strPath +"\\"+strBuildNum;
			strResultLocation = strBuildNumber+"\\"+scenarioName;
			strScreenshotLocation = strResultLocation+"\\screenshots";
			
			//Create BuildNumber dir
			objBuildNum = new File(strBuildNumber);
			if(!objBuildNum.exists()) {
				objBuildNum.mkdir();
			}
			...
			extent = new ExtentReports(strResultLocation+"\\"+fileName+"_"
					+appInd.getDataTime("ddMMYYYY_hhmmss")+".html", true);
		...
		try {
			switch(status.toLowerCase())
			{
				...
				case "warning":
					...
					break;
				case "screenshot":
					test.log(LogStatus.WARNING, description+": "+test.addScreenCapture(reports.getScreenshot(oDriver)));
					break;
				default:
